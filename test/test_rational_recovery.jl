...
@testset "affine-coordinate rational recovery" begin
    ...
    options = RSDP.RecoveryOptions(max_denominator = 100, atol = 1e-9, rtol = 1e-9)
    recovered =
        RSDP.recover_primal_certificate(problem, hint, options; return_diagnostics = true)
    ...
    wrong_objective = RSDP.NumericalPrimalHint([1 / 3, 2 / 3]; objective = 9.0)
    objective_error = try
        RSDP.recover_primal_certificate(
            problem,
            wrong_objective;
