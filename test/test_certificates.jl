...
@testset "exact primal certificates" begin
    ...
    affine_failure = RSDP.PrimalCertificate(
        certificate.problem_hash,
        Q[1 // 2, 1 // 3],
        Q(7 // 6),
    )
    affine_report =
        RSDP.check_certificate(problem, affine_failure; diagnostics=true)
    ...
    objective_failure = RSDP.PrimalCertificate(
        certificate.problem_hash,
        certificate.x,
        0,
    )
    ...
    no_objective =
        RSDP.make_primal_certificate(problem, x; include_objective=false)
